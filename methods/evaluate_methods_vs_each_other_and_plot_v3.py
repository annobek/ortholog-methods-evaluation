...
METHODS_CALCULATE = {
    "Prot-syn (n-score)": "/storage/EasyVectorOmics/synteny_algorithm/results/mammalian/one_to_one_nscore.tsv",
    "Prot-syn (sum)": "/storage/EasyVectorOmics/synteny_algorithm/results/mammalian/one_to_one_sum.tsv",
...
# Output settings
OUT_METRICS = "results/evaluation/mammalia/all_vs_all/combined_jaccard_matrix_v3.txt"
OUT_PLOT = "results/evaluation/mammalia/all_vs_all/combined_jaccard_heatmap_v3.png"
...
def plot_jaccard_heatmap(matrix, method_names, title, output_file, show_lower=False, 
                         completeness_methods=['BUSCO', 'Compleasm']):
    """
    Create and save a Jaccard similarity heatmap with separate mini heatmap for completeness tools.
    ...
    completeness_methods : list
        Methods to separate into mini heatmap (default: BUSCO, Compleasm)
    """
    # Separate orthology methods from completeness methods
    ortho_indices = [i for i, name in enumerate(method_names) if name not in completeness_methods]
    comp_indices = [i for i, name in enumerate(method_names) if name in completeness_methods]
    
    ortho_names = [method_names[i] for i in ortho_indices]
    comp_names = [method_names[i] for i in comp_indices]
    
    # Extract sub-matrices
    ortho_matrix = matrix[np.ix_(ortho_indices, ortho_indices)]
    
    if len(comp_indices) > 0:
        comp_matrix = matrix[np.ix_(comp_indices, comp_indices)]
    ...
    # ===== MAIN HEATMAP (Orthology Methods) =====
    n_ortho = len(ortho_names)
    im_main = ax_main.imshow(ortho_matrix, cmap="coolwarm", vmin=0, vmax=1)
    
    # Set ticks and labels
    ax_main.set_xticks(range(n_ortho))
    ax_main.set_yticks(range(n_ortho))
    ax_main.set_xticklabels(ortho_names, rotation=45, ha="right")
    ax_main.set_yticklabels(ortho_names)
    ...
    cbar_main = plt.colorbar(im_main, ax=ax_main, label="Jaccard Similarity")
    
    # Set title
    ax_main.set_title(title, fontsize=14, pad=20)
    
    # ===== MINI HEATMAP (Completeness Methods) =====
    if len(comp_indices) > 0:
        n_comp = len(comp_names)
        im_mini = ax_mini.imshow(comp_matrix, cmap="coolwarm", vmin=0, vmax=1, aspect='auto')
        
        # Set ticks and labels
        ax_mini.set_xticks(range(n_comp))
        ax_mini.set_yticks(range(n_comp))
        ax_mini.set_xticklabels(comp_names, rotation=45, ha="right")
        ax_mini.set_yticklabels(comp_names)
        ...
        for i in range(n_comp):
            for j in range(n_comp):
                if show_lower or i <= j:
                    ...
                    if not np.isnan(value):
                        ax_mini.text(j, i, f"{value:.2f}", 
                                   ha="center", va="center", color="black", fontsize=10)
        
        # Add colorbar for mini heatmap
        cbar_mini = plt.colorbar(im_mini, ax=ax_mini, label="Jaccard Similarity")
        
        # Set title
        ax_mini.set_title("Jaccard Similarity of BUSCO and compleasm", fontsize=12, pad=15)
